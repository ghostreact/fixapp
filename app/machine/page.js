...
export default function CreateMachineForm() {
  ...
  const onSubmit = (data) => {
    const payload = {
      ...data,
      machine_meet: isMeetingLinked, // เพิ่มค่าของการนัดหมาย
    };

    if (!isMeetingLinked) {
      delete payload.meetId; // ถ้าไม่มีการนัดหมาย ให้ลบ meetId ออกจาก payload
    }
    ...
    fetch("/api/machine", {
      ...
      .then((result) => {
        console.log(result);
        reset(); // รีเซ็ตฟอร์มหลังจากบันทึกสำเร็จ
      })
      .catch((error) => {
  ...
  return (
    <div className="m-5 border border-gray-400 border-spacing-3 rounded-lg p-4">
      ...
      >
        <div>
          <label htmlFor="name">ชื่องาน || ชื่อเจ้าของเครื่อง</label>
          <input
            type="text"
            id="name"
        ...
        <div>
          <label htmlFor="machine_tel">เบอร์ติดต่อ</label>
          <input
            type="text"
            id="machine_tel"
            {...register("machine_tel", { required: true })} // กำหนดเบอร์ติดต่อให้เป็น input field
            className="block w-full border border-gray-300 rounded-md shadow-sm p-2"
          />
          {errors.machine_tel && (
            <span className="text-red-500">กรุณากรอกเบอร์ติดต่อ</span>
          )}
        </div>
